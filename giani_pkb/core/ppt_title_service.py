...
def prepare_length_and_style_inputs_with_variance_handling(
    ...
    """
    Determines target length range and topicality, with robust handling for high variance.
    If variance is high, it defaults to the style of the LONGER of the two previous statements.
    """
    ...
    # --- Path 1: Both previous statements exist ---
    if len1 is not None and len2 is not None:
        ...
        if is_variance_high:
            # VARIANCE IS HIGH: Default to the style of the LONGER statement
            if max_len <= SHORT_WORD_COUNT_UPPER_BOUND:
                return f"{max(1, max_len - 2)}-{max_len + 2} words", True  # Topical
            else:
                return f"{max(1, max_len - 3)}-{max_len + 3} words", False # Insightful/Long
        else:
            # VARIANCE IS LOW: Use the average
            avg_len = (len1 + len2) / 2
            if avg_len <= SHORT_WORD_COUNT_UPPER_BOUND:
                return f"{max(1, int(avg_len) - 2)}-{int(avg_len) + 2} words", True # Topical
            else:
                return f"{max(1, int(avg_len) - 3)}-{int(avg_len) + 3} words", False # Insightful/Long

    # --- Path 2: Only one previous statement exists ---
    ...
    if single_prev_len is not None:
        if single_prev_len <= SHORT_WORD_COUNT_UPPER_BOUND:
            return f"{max(1, single_prev_len - 2)}-{single_prev_len + 2} words", True # Topical
        else:
            return f"{max(1, single_prev_len - 3)}-{single_prev_len + 3} words", False # Insightful/Long

    # --- Path 3: No previous statements, fallback to current ---
    else:
        ...
        if current_len is not None:
            if current_len <= SHORT_WORD_COUNT_UPPER_BOUND:
                return f"{max(1, current_len - 2)}-{current_len + 2} words", True # Topical
            else:
                return f"{max(1, current_len - 3)}-{current_len + 3} words", False # Insightful/Long
        else:
            # Ultimate fallback if no context for length at all
            return DEFAULT_LONG_RANGE_STR, False # Insightful/Long
...
def generate_titles(payload):
    try:
        ...
        # Use the improved length and style logic
        ...
        print(f"Suggestions: {output['suggestedTitles']}")
        return output
