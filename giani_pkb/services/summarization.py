...
class SummarizationService:
    ...
    def extract_clean_json(self, raw: str) -> Optional[str]:
        ...
        if "```json" in cleaned:
            ...
            end = cleaned.find("```", start)  # Find closing ``` after the opening
            cleaned = cleaned[start:end].strip() if end != -1 else cleaned[start:].strip()
        elif "```" in cleaned:
            start = cleaned.find("```") + 3  # Skip the opening ```
            end = cleaned.find("```", start + 1)  # Find closing ``` after the opening
            cleaned = cleaned[start:end].strip() if end != -1 else cleaned[start:].strip()
    ...
    def call_llm_api(self, prompt: str, prompt_type: str, retries: int = 3) -> Optional[Dict[str, Any]]:
        ...
        for attempt in range(retries):
            try:
                ...
                # Only validate metadata responses, skip validation for summarization
                if prompt_type == "metadata":
                    ...
                    if "llm_used_for_processing" not in result:
